...
// Autor: Giovanni Schrickte Sartori
...
void Principal::Menu()
{
	...
	while (op != 5)
	{
		...
		if (cin.fail()) {
			...
			cin.ignore(1000, '\n'); // descarta o lixo que ficou no buffer
			op = -1;               // garante que cai no default
		}
		switch (op)
		{
		case 1:
...
void Principal::RecuperarDepartamentos() {
	cont_idDepart += LDepartamentos.recupereDepartamentos(LUniversidades);
}
void Principal::RecuperarDisciplinas() {
	cont_idDisc += LDisciplinas.recupereDisciplinas(LDepartamentos);
}
void Principal::RecuperarAlunos() {
	cont_idAluno += LAlunos.recupereAlunos(LDisciplinas);
}